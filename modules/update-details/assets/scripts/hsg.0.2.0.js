 ...
 * Version 0.2.0
 * Created on 17-12-2014 03:12 by Yariv Arbiv
 */
;(function(core) {
  ...
  HSG.component("modal", {
    ...
    show: function() {
      ...
      $html.one("keydown.modal.hsg", function(e) {
        if ($this.isActive() && e.keyCode === 27 && $this.options.keyboard) {
          e.preventDefault();
          $this.hide();
        }
...
;(function($, HSG) {
  ...
  HSG.component("tableFade", {

    cls: {
      content: "hsg-table-responsive-content",
      fadeLeft: "hsg-fade-left",
      fadeRight: "hsg-fade-right"
    },
    ...
    init: function() {
      ...
      if (!this.element.data("tableFade")) return;

      this.element.trigger("hsg.tablefade.initiating");

      this.elementContent = this.element.find("." + this.cls.content);

      if (this.elementContent.length === 0) return;
      ...
      $win.on("resize orientationchange", (function() {
        ...
        $(function() {
          fn();
          $win.on("load", fn);
          $this.elementContent.on("scroll", fn);
        });

        return HSG.utils.debounce(fn, 150);
      })());

      this.element.trigger("hsg.tablefade.init");

      return this;
    },

    hasScrollBar: function() {
      if (this.elementContent.length === 0) return false;

      return this.elementContent.get(0).scrollHeight < this.elementContent.height();
    },

    calculate: function() {
      var contentScrollLeft = 0,
        contentClientWidth = 0,
        contentScrollWidth = 0;

      if (!this.hasScrollBar()) {
        this.element.removeClass(this.cls.fadeLeft + " " + this.cls.fadeRight);
        return;
      }

      this.element.addClass(this.cls.fadeLeft + " " + this.cls.fadeRight);

      contentScrollLeft = this.elementContent.scrollLeft();
      contentClientWidth = this.elementContent.get(0).clientWidth;
      contentScrollWidth = this.elementContent.get(0).scrollWidth;

      // scroll to left
      if (contentScrollLeft === 0) {
        this.element.removeClass(this.cls.fadeLeft);
      }

      // scroll to right
      if (contentScrollWidth - contentClientWidth === contentScrollLeft) {
        this.element.removeClass(this.cls.fadeRight);
      }
    }
  });
  ...
  // attach events
  $(document).on("hsg-domready", function(e) {
    $("[data-hsg-table-fade]").each(function() {
      var obj, ele = $(this);

      if (!ele.data("tableFade")) {
        obj = HSG.tableFade(ele, HSG.utils.options(ele.attr("data-hsg-table-fade")));
      }
    });
  });
